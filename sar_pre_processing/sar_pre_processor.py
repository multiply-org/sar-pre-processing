...
class SARPreProcessor(PreProcessor):
    ...
    def pre_process_step1(self, **kwargs):
        ...
        for file in self.file_list:
            ...
            if process_all == 'no':
                pdb.set_trace()
                subprocess.call(self.config.gpt + ' ' + os.path.join(self.config.xml_graph_path, self.config.xml_graph_pre_process_step1) + ' -Pinput="' + file + '" -Poutput="' + outputfile + '" -Pangle="' + str(normalisation_angle) + '" -Parea="POLYGON ((' + area + '))" -c 2G -x', shell=True)
            else:
                subprocess.call(self.config.gpt + ' ' + os.path.join(self.config.xml_graph_path, self.config.xml_graph_pre_process_step1) + ' -Pinput="' + file + '" -Poutput="' + outputfile + '" -Pangle="' +str(normalisation_angle) + '" -c 2G -x', shell=True)
    ...
    def pre_process_step3(self, **kwargs):
        ...
        # Check if output folder for step3 is specified, if not existing create new folder
        ...
        if self.config.speckle_filter.multi_temporal.apply == 'yes':
            ...
            for i, file in enumerate(filelist):
                ...
                filepath, filename, fileshortname, extension = self._decomposition_filename(file)
                a,a,b,a = self._decomposition_filename(self._create_filelist(os.path.join(filepath,fileshortname+'.data'), '*_slv1_*.img')[0])
                a,a,c,a = self._decomposition_filename(self._create_filelist(os.path.join(filepath,fileshortname+'.data'), '*_slv2_*.img')[0])
